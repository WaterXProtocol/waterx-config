// The legacy→target field map as CODE — the ONE implementation of the lift,
// used by the parser (pre-flip: lift the served legacy document into the
// target shape) and by scripts/derive-target.mjs (flip-day generation + CI
// losslessness gate). Every legacy field is either moved here or is a known
// dead forensics field; anything else THROWS in strict mode, so a legacy
// addition cannot silently miss the flip story.
//
// Mirrored by packages/rust/src/lift.rs; the two are held equal by the
// cross-language test in the Rust crate (lift output compared value-for-value
// against this implementation's CI-emitted output).

const IDENTITY = ["published_at", "original_id", "version", "upgrade_capability", "mvr"];
// Deploy forensics were pruned from the live files (audit Phase 2; archived in
// deploys/). If one reappears, strict mode throws via leftover detection.

/**
 * Lift a legacy-shape document into the target shape.
 * @param v1 the legacy document
 * @param opts.strict throw on any legacy field with no disposition (default
 *   true — repo tooling wants flip completeness). The parser passes false:
 *   a consumer must tolerate a legacy field added after its pinned version
 *   (forward-compat), same policy as unknown fields on target-shape docs.
 */
export function liftToTarget(v1, opts = {}) {
  ...
  // ── symbols (kind moves here) + venue_feeds in one pass ─────────────────
  const feedsSrc = take("waterx_rule", "feeds") ?? {};
  const symbols = {};
  const venueFeeds = {};
  for (const [sym, { kind, ...rest }] of Object.entries(feedsSrc)) {
    symbols[sym] = { kind: kind ?? "perp" };
    venueFeeds[sym] = rest;
  }
  ...
  oracle_rules.waterx = {
    ...
    enclave: {
      ...
      // sole config home (audit correction: the enclave package block never duplicated it)
      pubkey: take("waterx_rule", "enclave_pubkey"),
    },
    venue_feeds: venueFeeds,
  };
  if (P.pyth_rule) {
    oracle_rules.pyth = {
      package: "pyth_rule",
  ...
  put("bridge", "wormhole_state", take("wormhole_bridge", "wormhole_state"));
  {
    ...
    for (const [from, to] of [
      ...
      const v = take("wormhole_bridge", from);
      if (v !== undefined) limits[to] = v;
    }
  ...
  if (strict) {
    ...
    if (leftovers.length) {
      throw new Error(`legacy fields with no flip disposition (extend lift.mjs AND lift.rs): ${leftovers.join(", ")}`);
    }
  }
