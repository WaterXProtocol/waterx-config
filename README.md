...
| `usd_credit`, `sui_credit`, `deep_credit`, `wal_credit` | `metadata_cap` — the credit coin packages, each `<asset>_credit::<asset>::<ASSET>` (`…::usd::USD`, `…::sui::SUI`, `…::deep::DEEP`, `…::wal::WAL`). While a coin is published but its registry not yet created the block also carries `proof` (the one-shot `create_credit_registry` input) |
